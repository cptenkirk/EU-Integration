...
function initializeMapLayout(map, geojsonData) {
    ...
    capitals.forEach(capital => {
        ...
        // --- Funktionen für Maus-Interaktionen ---
        ...
        function resetHighlight(e) {
            var layer = e.target;
            geojsonLayer.resetStyle(e.target); // Stil auf den ursprünglichen Zustand zurücksetzen

            // Das Land wieder in den Hintergrund schieben, damit andere Länder darüber erscheinen können
            // Wichtig: Wir müssen sicherstellen, dass die Labels (Städtenamen etc.) immer oben bleiben.
            layer.bringToBack();
            map.getPane('labels').style.zIndex = 650; // Stellt sicher, dass die Labels nicht verdeckt werden
        }
        ...
        function handleCountryClick(e) {
            ...
            var countryName = e.target.feature.properties.name_de || e.target.feature.properties.name;
            if (countryName) {
                // Call the global function defined in index2.html
                if (typeof openCountryInfoDialog === 'function') {
                    openCountryInfoDialog(countryName);
                } else {
                    console.warn("openCountryInfoDialog function not found. Make sure it's defined globally in index2.html.");
                }
            }
        }
        // Diese Funktion wird für jedes Land aufgerufen
        function onEachFeature(feature, layer) {
            // Pop-up mit dem Ländernamen hinzufügen
            if (feature.properties && feature.properties.name_de) {
                layer.bindPopup(feature.properties.name_de);
            }
            ...
            layer.on({
                ...
                //click: handleCountryClick // Use the new handler here
            });
        }

        var geojsonLayer; // Variable für den GeoJSON-Layer deklarieren

        // Vektor-Karte (jetzt mit den übergebenen geojsonData)
...
export function openCountryInfoDialog(countryName, characterName) {
            ...
            content.innerHTML = `<p>Informationen über <strong>${countryName}</strong>.</p>
                                 <div class="loader"></div>
                                 <p style="color: #666; margin-top: 1rem;">Lade KI-Informationen...</p>
                                 <p style="color: #666; margin-top: 1rem;">Diese Box ist 800px breit und 600px hoch.</p>`; // Behalte die Größeninfo bei
            ...
            if (typeof getAIResponseForCountry === 'function') {
                        ...
                        function typeWriter() {
                            if (i < displayText.length) {
                                ...
                                if (sourceUrl) {
                                    ...
                                    qrcodeContainer.innerHTML = '<p style="color: #000000ff; margin-top: 1rem; font-size: 20px;">Für mehr Informationen, scannen sie bitte den QR-Code.<br></p>'; // Füge den Hinweistext hinzu
                ...
                content.innerHTML = `<p>Informationen über <strong>${countryName}</strong>.</p>
                                     <p style="color: red; margin-top: 1rem;">KI-Funktion nicht verfügbar.</p>
                                     <p style="color: #666; margin-top: 1rem;">Diese Box ist 800px breit und 600px hoch.</p>`;
            }
        }
...
export function initializeMap(L, geojsonData) {
    ...
    map.setMaxBounds([
        [65, -80], // Nordwesten (weiter nach oben und links)
